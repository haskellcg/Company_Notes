...
## 27. 配置EFM和BFD联动
  EFM和BFD联动为双向联动，即
  * 当EFM检测到链路出现故障时，会联动BFD，将故障通知到远端设备
  * 当BFD检测到链路出现故障时，会联动EFM，将故障通知到远端设备

  BFD支持MGR联动的类型如下：
  * BFD for LSP (LDP、TE、TE-LSP、Static-LSP、VLL PW)静态会话，但需要配置PST
  * BFD for IP类型的BFD会话，但不需要配置PST

## 28. 配置PIM BFD
  当BFD检测到对端故障以后上报PIM模块，PIM模块立即触发新一轮的DR竞选过程，而不是等到邻居关系超时，这将很大程度上缩小组播数据传输的中断时间，提高组播网络的可靠性。

  在PIM协议运行过程中，PIM邻居间大连路状态的变化会触发某些工作机制(如DR选举、Assert Winner选举)重新进行。比如共享网段上的当前DR发生故障，其他PIM邻居会等到邻居关系超时才触发新一轮的DR竞选过程，导致组播数据传输中断，中断的时间将不小于邻居关系的超时时间，通常时秒级。

  PIM BFD能够在毫秒级内检测共享网段内的链路状态，快速响应PIM邻居故障。如果配置了PIM BFD功能的接口在检测周期内没有收到DR发送的BFD检测报文，则认为当前DR发生故障，BFD快速把会话状态通告给路由管理模块(RM)，再由RM通告给PIM。PIM模块触发新一轮的DR竞选过程，而不是等到邻居关系超时，从而减少组播数据传输的中断时间，提高组播数据传输的可靠性。

  PIM BFD也使用与共享网段上Assert竞选的过程，可以快速响应Assert Winner接口故障。
